...
class MY_Pagination extends CI_Pagination {
	...
	function __construct() {
		$this->num_links = 4;
		$this->cur_page = 1;
		$this->first_link = '&lsaquo;';
		$this->last_link = '&rsaquo;';
		$this->uri_segment = 5;
			...
			for ($loop = $start -1; $loop <= $end; $loop++)
			{
				...
				if ($i >= $base_page)
				{
					...
					else
					{
						...
						if ($n == '' && $this->first_url != '')
						{
                            $output .= $this->num_tag_open.'<a '.$this->anchor_class.'href="'.$this->base_url.$this->prefix.$n.$this->suffix.'"'.($this->onclick?on_click($this->onclick, 1):'').'>'.$loop.'</a>'.$this->num_tag_close;
						}
						else
						{
