...
class Login extends CI_Controller {
	...
	function otpReg(){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['otp'] = $this->otp->ci_generator($member_id, 'CONUN');
			...
			if( $this->otp->ci_verify($secretKey,$otp_password)){
				...
				alert("opt 등록되었습니다",'https://www.liumwallet.com');
	...
	function index()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if ($lang == "kr" or $lang == "") {
				layout('app/login_KR',$data,'app_main');
			}
		
			else if ($lang == "us" ) {
				layout('app/login',$data,'app_main');
			}

			else if ($lang == "cn") {
				layout('app/login_CN',$data,'app_main');
			}
			

			
			else if ($lang == "jp") {
				layout('app/login_JP',$data,'app_main');	
			}
