...
<div id="util">
    ...
    <li style="display:inline"><a href="javascript:multi_recipt();">정산완료</a></ii>
...
<div  style="width: 100%;height: 90px">
    <form name='fsearch' method='post' action="/admin/new_coin/payment_list">
        ...
        <div style="margin-bottom: 5px;">
            ...
            <lable style="margin-right: 60px">결제일</lable>  <input class="input-find datepicker" style="width: 100px"  autocomplete=”off” type="text" name="from_date"  value="<?=$from_date;?>"> ~ <lable> </lable><input class="input-find datepicker" autocomplete=”off” style="width: 100px"  type="text" name="to_date" value="<?=$to_date;?>">
...
<div id="table">
    <table id="lists" summary="회원 리스트">
        ...
        if($row->type =="CON") {
            $total_amount = $total_amount +  round($row->payment_price*$row->payment_amount);
        ...
        <Tr>
            ...
            <td></td>
            <td></td>
            <td><span class="bold"><?=number_format($total_amount,2);?>원</span></td>
            <td></td>
            <td></td>
...
<script>
    ...
    function multi_recipt(){
        var valuesArray = $('input[name="coin_no"]:checked').map(function () {
            return this.value;
        }).get().join(",");

        if(valuesArray!=""){
            if(confirm("정산완료 처리 하시겠습니까?")) {
                var del = document.del;
                del.idx.value = valuesArray;
                del.action = "/admin/coin/recipt";
                del.submit();
            }
        }else{
            alert("선택한 내용이 없습니다.");
