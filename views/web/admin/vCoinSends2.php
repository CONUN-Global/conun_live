<?=$this->load->view('admin/layout/windowHead')?>

<? echo form_open_multipart(uri_string(), 'id="frm"'); ?>
    <input type="hidden" name="sender_id" value="<?=implode(",",$sender_id);?>">
    <input type="hidden" name="sender_email" value="<?=implode(",",$sender_email);?>">
    <input type="hidden" name="sender_amount"  id="sender_amount" value="">
    <input type="hidden" name="coin"  id="coin" value="<?=$coin;?>">
    <table id="his">

        <thead  >
        <tr>
            <th>주소</th>
            <th>LIT 잔고</th>
            <th>ETH 잔고</th>
            <th>수량</th>
        </tr>
        <tr><td colspan="4" class="line"></td></tr>
        </thead>
        <tbody  >
        <?
        if(empty($sender_name)===false){
            foreach($sender_name as $key=>$item){?>

                <tr>

                    <td class="txt" width="50" style="font-size: 10px"><?=$sender_email[$key];?></td>
                    <td class="txt" width="50" style="font-size: 10px"><?=number_format($sender_lit_bal[$key],4);?></td>
                    <td class="txt" width="50" style="font-size: 10px"><?=number_format($sender_eth_bal[$key],4);?></td>
                    <td class="txt" width="100"><input type="text" name="amount" class="amount numberformat "  style="width: 80px"> </td>
                </tr>
                <tr><td colspan="4" class="line"></td></tr>
            <? }} ?>


        </tbody>
    </table>
    <table id="write">
        <colgroup></colgroup>
        <tbody>

        <tr>
            <td class="tit">주소</td>
            <td class="txt"><input type="text" name="TO_ADDR" required itemname="TO_ADDR" style="width: 100%" value=""></td>
        </tr>

        <tr><td colspan="3" class="line"></td></tr></tbody>
    </table>
    <div class="btn"><input type="button" class="btn_01" value="코인지급하기" id="btn_submit"></div>
    </form>

    <script>
        $(document).ready(function () {

            $('.numberformat').removeText();
            $("#btn_submit").click(function () {
                var check =  true;
                $(".amount").each(function () {

                    if($(this).val() == ""){

                        alert("수량을 입력 하세요.");
                        $(this).focus();
                        check = false;
                    }

                });

                if($("#TO_ADDR").val() ==""){
                    alert("주소을 입력 하세요.");
                    check = false;
                }

                if(check==true){
                    var valuesArray = $('input[name="amount"]').map(function () {
                        return this.value;
                    }).get().join(",");

                    $("#sender_amount").val(valuesArray);

                    $("#frm").submit();
                }

            });

        });

    </script>

<?=$this->load->view('admin/layout/windowFoot')?>